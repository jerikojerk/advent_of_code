...
class LigthAction:
    ...
    def count_impact(self):
        return (self.end[1]+1-self.start[1])*(self.end[0]+1-self.start[0])

    def _turn_on(self,grid):
        cnt = self.count_impact()
...
for instruction in instructions:
    la = LigthAction(instruction)
    cnt = la.apply(grid)
    print(  instruction , " impacts ", cnt , "ligths" )

result=sum(grid.values())
result2=len(grid)
print( "hello santa,   total brightness is ",result," over ",result2," lights." )
